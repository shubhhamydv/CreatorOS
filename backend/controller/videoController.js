...
export const createVideo = async (req, res) => {
  try {
    ...
    if (!title || !req.files?.video || !req.files?.thumbnail || !channelId)
      return res.status(400).json({ message: "title, video, thumbnail, channelId is required" })

    const channelData = await Channel.findById(channelId)
    if (!channelData) return res.status(400).json({ message: "Channel not found" })
    ...
    if (!uploadedVideo || !uploadedThumbnail)
      return res.status(500).json({ message: "Failed to upload to Cloudinary. Check CLOUDINARY env vars." })

    let parseTag = []
    if (tags) { try { parseTag = JSON.parse(tags) } catch { parseTag = [] } }

    const newVideo = await Video.create({
      title, channel: channelData._id, description,
...
export const getAllVideos = async (req, res) => {
  try {
    ...
    if (!videos) return res.status(400).json({ message: "Videos are not found" })
...
export const toggleLikes = async (req, res) => {
  try {
    ...
    if (!video) return res.status(400).json({ message: "Video is not found" })
...
export const toggleSave = async (req, res) => {
  try {
    ...
    const video = await Video.findById(videoId)
    if (!video) return res.status(400).json({ message: "Video is not found" })
    if (video.saveBy.includes(userId)) video.saveBy.pull(userId)
    else video.saveBy.push(userId)
    await video.save()
    return res.status(200).json(video)
  } catch (error) {
...
export const getViews = async (req, res) => {
  try {
    ...
    if (!video) return res.status(400).json({ message: "Video is not found" })
    return res.status(200).json(video)  // FIX: 500 → 200
  } catch (error) {
    return res.status(500).json({ message: `Failed to get views ${error}` })
  }
}

// FIX: was missing await video.save()
export const addComment = async (req, res) => {
  try {
    ...
    await video.save()  // FIX: was missing
    return res.status(200).json(video)
  } catch (error) {
...
// FIX: completely rewritten - had unreachable code, wrong var names
