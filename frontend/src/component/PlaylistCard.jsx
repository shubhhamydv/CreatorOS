...
const PlaylistCard = ({id, title, videos, savedBy}) => {
    ...
    const handleSave = async ()=>{
        try {
            const result = await axios.post(serverUrl + "/api/content/playlist/toggle-save",
            { playtlistId : id} , {withCredentials:true})
  ...
  return (
    <div className='relative w-60 h-40 rounded-xl overflow-hidden group shadow-lg bg-gray-900'>
        ...
        <h3 className='font-semibold text-white truncate'>{title}
        </h3>
        ...
        ${
            ...
            `} onClick={handleSave}><FaBookmark size={16}/>
        </button>
        <button className='absolute botton-2 right-2 bg-black/70 p-2 rounded-full text-white hover:bg-black transition'
    ...
    {showVideos &&(
        ...
        <button className='absolute top-4 right-4 text-gray-400 hover:text-white transition'
         onClick={()=>setShowVideos(false)}><FaTimes size={24}/></button>
         ...
         <div className='h-[2px] items-center justify-around gap-5 flex-wrap'> 
         <VideoCard
         key={v._id}
         id={v._id}
