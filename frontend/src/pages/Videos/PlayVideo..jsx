...
// FIX: icon→Icon uppercase so React treats it as a component
...
function PlayVideo() {
  ...
  useEffect(() => {
    ...
    const addViews = async () => {
      try {
        ...
        const updatedVideos = allVideosData.map((v) => v._id === videoId ? { ...v, views: result.data.views } : v)
  ...
  useEffect(() => {
    setIsSubscribed(channel?.subscribers?.some((sub) =>
      sub._id?.toString() === userData?._id?.toString() || sub?.toString() === userData?._id?.toString()
    ))
  ...
  const handleVolume = (e) => {
    ...
    setVol(newVol); setIsMuted(newVol === 0)
    if (videoRef.current) videoRef.current.volume = newVol
  }
  const handleMute = () => {
    if (!videoRef.current) return;
    setIsMuted(!isMuted); videoRef.current.muted = !isMuted
  }
    ...
    <div className='flex bg-[#0f0f0f] text-white flex-col lg:flex-row gap-6 p-4 lg:p-6'>
      <div className='flex-1'>
        <div onMouseEnter={() => setShowControls(true)} onMouseLeave={() => setShowControls(false)}
          ...
          {showControls && (
            <div className='absolute inset-0 hidden lg:flex items-center justify-center gap-6 z-20'>
              <button className='bg-black/70 p-3 rounded-full hover:bg-orange-600' onClick={skipBackward}><FaBackward size={24} /></button>
              <button className='bg-black/70 p-3 rounded-full hover:bg-orange-600' onClick={togglePlay}>{isPlaying ? <FaPause size={28} /> : <FaPlay size={28} />}</button>
              <button className='bg-black/70 p-3 rounded-full hover:bg-orange-600' onClick={skipForward}><FaForward size={24} /></button>
            </div>
          )}
          <div className='absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-4 py-2 z-30'>
            <input type="range" min={0} max={100} onChange={handleSeek} className='w-full accent-orange-600' value={progress} />
            <div className='flex items-center justify-between mt-1 text-xs text-gray-200'>
              <div className='flex items-center gap-3'>
                <span>{formatTime(currentTime)}/{formatTime(duration)}</span>
                <button onClick={skipBackward}><FaBackward size={14} /></button>
                <button onClick={togglePlay}>{isPlaying ? <FaPause size={14} /> : <FaPlay size={14} />}</button>
                <button onClick={skipForward}><FaForward size={14} /></button>
              </div>
        ...
        <h1 className='mt-4 text-lg font-bold text-white'>{video?.title}</h1>
        ...
        <div className='mt-2 flex flex-wrap items-center justify-between'>
          <div className='flex items-center gap-4'>
            ...
            <button onClick={handleSubscribe} className={`px-5 py-2 rounded-full border border-gray-600 ml-5 ${isSubscribed ? "bg-black text-white hover:bg-orange-600" : "bg-white text-black hover:bg-orange-600"}`}>
        ...
        <div className='mt-6'>
          ...
          <div className='flex gap-2 mb-4'>
            <input type="text" placeholder='Add a comment...' className='flex-1 border-gray-700 bg-[#1a1a1a] text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-orange-600' onChange={(e) => setNewComment(e.target.value)} value={newComment} />
            <button className='bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg' disabled={loading1} onClick={haddleAddComment}>{loading1 ? <ClipLoader size={20} color='black' /> : "Post"}</button>
          </div>
          <div className='space-y-3 max-h-[300px] overflow-y-auto pr-2'>
            {comment?.map((c) => (
              <div key={c?._id} className='p-3 bg-[#1a1a1a] rounded-lg text-sm'>
      ...
      <div className='w-full lg:w-[380px] px-4 py-4 border border-gray-800 overflow-y-auto'>
        ...
        <div className='space-y-3'>
          {suggestedVideos?.map((v) => (
            <div key={v._id} className='flex gap-2 cursor-pointer hover:bg-[#1a1a1a] p-2 rounded-lg' onClick={() => navigate(`/playvideo/${v._id}`)}>
              <img src={v?.thumbnail} className='w-32 h-20 rounded-lg object-cover' alt="" />
              <div>
                <p className='font-semibold line-clamp-2 text-sm text-white'>{v?.title}</p>
                <p className='text-xs text-gray-400'>{v?.channel?.name}</p>
