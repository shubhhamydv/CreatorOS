...
function PlayVideo() {
  ...
  useEffect(() => { setViewCounted(false) }, [videoId])
  ...
  useEffect(() => {
    ...
    const addViews = async () => {
      try {
        ...
        setVideo((prev) => prev ? { ...prev, views: updatedVideo.views } : prev)
        setViewCounted(true)
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      setChannel((prev) => ({ ...prev, subscribers: updatedChannel.subscribers || prev.subscribers }))
    } catch (error) { console.log(error) }
    setLoading(false)
  }
  ...
  const toggleLike = async () => {
    try {
      ...
      setVideo(result.data?.video || result.data)
    } catch (error) { console.log(error) }
  }
        ...
        <div className='mt-2 flex flex-wrap items-center justify-between'>
          <div className='flex items-center justify-start gap-4'>
            ...
            <button
              ...
              className={`px-[20px] py-[8px] rounded-full border border-gray-600 ml-[20px] text-md ${isSubscribed ? "bg-black text-white hover:bg-orange-600" : "bg-white text-black hover:bg-orange-600"}`}
            >
          ...
          <div className='flex items-center gap-6 mt-3'>
            ...
            <IconButton icon={FaBookmark} label={"Save"} active={video?.savedBy?.includes(userData?._id)} onClick={toggleSave} />
          </div>
        </div>
        ...
        <div className='mt-6'>
          ...
          <div className='flex gap-2 mb-4'>
            <input
              ...
              className='flex-1 border-gray-700 bg-[#1a1a1a] text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-red-600'
    ...
    <div className='mt-3'>
      {showReplyInput && (
        <div className='flex gap-2 mt-1 ml-4'>
          ...
          />
          <button
            onClick={() => { handleReply({ commentId: comment._id, replyText }); setShowReplyInput(false); setReplyText("") }}
            disabled={loading3}
