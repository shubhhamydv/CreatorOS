        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                ...
                if (entry.isIntersecting) {
                    video.muted = false
                    video.play().setActiveIndex(index)
            ...
            {shortList?.map((short, index) => (
                <div key={short?._id} className="min-h-screen flex justify-center items-center snap-start">
                    <div className="relative w-[350px] aspect-[9/16] bg-black rounded-2xl overflow-hidden" onClick={() => togglePlay(index)}>
                        <video ref={(el) => (shortRefs.current[index] = el)} data-index={index}
                            src={short?.shortUrl} className="w-full h-full object-cover" loop muted playsInline />
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent text-white">
                            <div className="flex items-center gap-2 mb-2">
                                <img src={short?.channel?.avatar} alt="" className="w-8 h-8 rounded-full"onClick={()=>navigate(`/channelpage/${short?.channel.id}`)} />
                                <span className="test-sm text-gray-300" onClick={()=>navigate(`/channelpage/${short?.channel.id}`)}>@{short?.channel?.name?.toLowerCase()}</span>
                                <button
